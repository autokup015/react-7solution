...
import { transformData } from "@/services/dummy";
...
describe("<DummyView />", () => {
  const mockDataResponse: TDummyResponse = {
    limit: 0,
    skip: 0,
    total: 30,
    users: [
  ...
  const setup = ({ isSuccess = true }: { isSuccess?: boolean }) => {
    (useQuery as Mock).mockImplementation((queyrKey) => {
      const endpoint = queyrKey.queryKey[1];

      if (endpoint === "https://dummyjson.com/users") {
        return {
          isSuccess,
          data: {
            ...mockDataResponse,
            users: transformData(mockDataResponse.users),
          },
        };
      }
    });

    const agrs = render(<DummyView />);
  ...
  it("should render DummyView", () => {
    setup({});

    expect(screen.getByTestId("pre-response")).toBeInTheDocument();
  });
